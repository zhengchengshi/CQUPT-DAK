...
export default function AddTeam() {
  ...
  const submit = () => {
    ...
    Taro.showLoading({
      mask: true,
      title: "已提交,我们会尽快进行审核,预计时间1天",
    });
    (async () => {
      const imgUrlArr = [];
      // 上传图床
      for (let i = 0; i < allImgs.length; i++) {
        const token = await api.get("/picture/upload-token").then((res) => {
      ...
      const option = {
        team_name: iptTitle.current.value,
        information: {
          qq_number: iptQqNum.current.value,
          vx_number: iptWxNum.current.value,
          qr_code: "",
        },
        description: iptDetail,
        images: imgUrlArr,
      };
      await api.post("/team/join", option).then((res) => {
        Taro.hideLoading({
          success: () => {
            Taro.navigateBack();
          },
  ...
  return (
    <View>
      <View className="addTeam-header">欢迎入驻!</View>
      <View className="addTeam-content">
        <Image
          src="https://s1.ax1x.com/2022/03/29/qcK3KP.png"
          className="addTeam-formdata-content-shadow"
        ></Image>
        <View className="addTeam-first-formdata">
          <View className="addTeam-formdata-content">
            <View className="addTeam-formdata-content-item">
              团队名称
              <Image
                src="https://s1.ax1x.com/2022/03/29/qcug1I.png"
                className="addTeam-formdata-content-item-required"
              ></Image>
            </View>
            <View className="addTeam-formdata-content-item">
              <Input ref={iptTitle}></Input>
            </View>
        ...
        <View className="addTeam-second-formdata">
          <View className="addTeam-connection">
            <View className="addTeam-connection-title">
              联系方式
              <View className="addTeam-connection-title-right">
                至少留一个哟~不然大家怎么联系到你
              </View>
            </View>
            <View className="addTeam-connection-item">
              <Text>QQ</Text>
              <Input placeholder="请填写QQ联系方式" ref={iptQqNum}></Input>
            </View>
            <View className="addTeam-connection-item">
              <Text>微信</Text>
              <Input placeholder="请填写微信联系方式" ref={iptWxNum}></Input>
            </View>
          </View>
          <View className="addTeam-others-connection">
            <View className="addTeam-others-connection-title">
              <Text>其他联系方式</Text>
              <Text
                className="addTeam-others-connection-add-connection-btn"
              ...
              return (
                <View className="addTeam-others-connection-addItem" key={index}>
                  <View className="addTeam-others-connection-addItem-btn">
                    <Image
                      src="https://s1.ax1x.com/2022/04/11/LZdikT.png"
                      className="addTeam-others-connection-addItem-icon"
                    ></Image>
                  </View>
        ...
        <View formType="submit" onClick={submit} className="addTeam-btn">
          发送
        </View>
      </View>
    </View>
